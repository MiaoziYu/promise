...
class PurchaseWishTest extends TestCase
{
    ...
    /** @test */
    public function can_update_credits_contributed_statistic()
    {
        // Arrange
        factory(UserProfile::class)->create([
            'user_id' => $this->user->id,
    ...
    /** @test */
    public function can_update_wish_tickets_amount_statistic_after_purchased_a_wish()
    {
        // Arrange
        factory(UserProfile::class)->create([
            'user_id' => $this->user->id,
        ...
        $this->assertEquals(1, $this->user->userProfile->wish_tickets_amount);
    }

    /** @test */
    public function can_update_wish_tickets_amount_statistic_after_contribute_to_a_shared_wish()
    {
        // Arrange
        factory(UserProfile::class)->create([
            'user_id' => $this->user->id,
        ...
        // Act
        $response = $this->actingAs($this->user)->put('/api/wishes/' . $wish->id . '/contribute?api_token=' . $this->user->api_token, [
            'credits' => 100
        ]);
